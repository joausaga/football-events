...
In this part of the study, I am interested in further understanding the attempt events, specially those that ended up as goals. Through the next plots I will explore the relationship between teams and attempts, teams and goals, goals and assitance method, goals and time, goals and body part, goals and game situation, and goals and location. Also, I will investigate the team's fair play.

### Teams and attempts

```{r echo=FALSE, Bivariate_Plots}
teams_attempt = events %>% filter(event_type == 'Attempt') %>% select(event_team, event_type)
t_teams_attempt = data.frame(table(teams_attempt, useNA="ifany"))
t_teams_attempt = arrange(t_teams_attempt, desc(Freq))
ggplot(aes(x=reorder(event_team, -Freq), y=Freq, fill=event_team), data=t_teams_attempt[1:10,]) +
  geom_bar(colour="black", stat = "identity") +
  geom_text(aes(label=Freq), position=position_dodge(width=0.9), vjust=-0.50, size=4) +
  guides(fill=FALSE) +
  theme_economist() +
  xlab('Top-10 teams with more attempts') +
  ylab('Count') +
  theme(axis.text.x  = element_text(angle=90,vjust=0.5, size=10)) +
  ylim(0, 4000)
```
...
teams_goal = events %>% filter(event_type == 'Attempt' & is_goal == 1) %>% select(event_team, event_type, is_goal)
t_teams_goal = data.frame(table(teams_goal, useNA="ifany"))
t_teams_goal = arrange(t_teams_goal, desc(Freq))
ggplot(aes(x=reorder(event_team, -Freq), y=Freq, fill=event_team), data=t_teams_goal[1:10,]) +
  geom_bar(colour="black", stat = "identity") +
  geom_text(aes(label=Freq), position=position_dodge(width=0.9), vjust=-0.50, size=4) +
  guides(fill=FALSE) +
  theme_economist() +
  xlab('Top-10 teams with more goals') +
  ylab('Count') +
  theme(axis.text.x  = element_text(angle=90,vjust=0.5, size=10)) +
  ylim(0, 600)
```
...
ggplot(aes(x=reorder(team,per_fouls), y=per_fouls, fill=team), data=teams_fouls[1:10,]) +
  geom_bar(colour="black", stat = "identity") +
  geom_text(aes(label=round(per_fouls,3)), position=position_dodge(width=0.9), vjust=-0.50, size=4) +
  guides(fill=FALSE) +
  theme_economist() +
  xlab('Top-10 teams that conceded less fouls') +
  ylab('Ratio of fouls by team\'s events') +
  theme(axis.text.x  = element_text(angle=90, vjust=0.5, size=10)) +
  ylim(0, 0.25)
```
...
In this section, I'll explore some characteristics of the teams' performance. , like the effectiveness,  the effectiveness of teams. By effectiveness, I mean number of goals by number of attempts.
...
ggplot(aes(x=total_attempts, y=goals), data=teams_effectiveness) +
...
Green dots represent the top-10 most effective teams. The team effectiveness is specified in parenthesis below the team name.
